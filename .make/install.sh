...
BINARY_NAME="${BINARY_NAME:-biorhythm}"
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$ROOT_DIR/.venv/bin/$BINARY_NAME"
...
if [ ! -x "$SRC" ]; then
  echo "error: missing entry point: $SRC" >&2
  echo "hint:  run make setup first (and define [project.scripts])" >&2
  exit 1
fi
